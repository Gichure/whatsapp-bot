...
""", use_aliases=True)
            ...
            r = requests.put('https://api.apify.com/v2/actor-tasks/6Mmi6fuHrtJTjsbzA/input?token=jMi2xYyMJPyNYpdFQ6CNEvfBi=1', data = data, headers={"content-type": "application/json"})
            if r.status_code != 200:
                result = 'Sorry, I cannot search for recipes at this time.'

            # runs task to scrape Allrecipes for the top 5 search results
            r = requests.post('https://api.apify.com/v2/actor-tasks/6Mmi6fuHrtJTjsbzA/runs?token=jMi2xYyMJPyNYpdFQ6CNEvfBi=1')
...
""".format(
    ...
    last_updated.second
            ...
            r = requests.get('https://www.reddit.com/r/memes/top.json?limit=20?t=day', headers = {'User-agent': 'Happy Bot 0.1'})
            if r.status_code == 200:
                data = r.json()
            ...
            responded = True    
        if not responded:
             msg.body("Sorry, I didn't understand. Send 'Menu' for a list of commands.")
